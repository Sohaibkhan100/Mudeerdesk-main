...
class Logger:
    exit_event = threading.Event()  # Event to signal the thread to exit
    logging_thread = None  # Global variable to store the thread
    timer_instance = None  # Store timer instance to update


def on_press(key):
    try:
        print(f"Key pressed: {key.char}")
        if Logger.timer_instance:
            Logger.timer_instance.ispressed = "true"
            Logger.timer_instance.employee_status = 1
    except AttributeError:
        print(f"Special key pressed: {key}")
        if Logger.timer_instance:
            Logger.timer_instance.ispressed = "true"
...
def set_timer_instance(timer_instance):
    """Set the timer instance for key logger to update"""
    Logger.timer_instance = timer_instance